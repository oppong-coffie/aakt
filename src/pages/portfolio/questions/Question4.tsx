...
const Question4 = () => {
  ...
  const [entries, setEntries] = useState<CauseEffect[]>([
    { cause: "", effect: "" },
  ]);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const addEntry = () => {
    setEntries([...entries, { cause: "", effect: "" }]);
  };

  const updateEntry = (
    index: number,
    field: keyof CauseEffect,
    value: string,
  ) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f0f0eb] font-sans relative">
      ...
      <div className="flex-1 px-6 sm:px-12 overflow-y-auto pb-32">
        ...
        <button
          onClick={addEntry}
          className="flex items-center gap-2 text-blue-600 font-medium mb-8 hover:text-blue-700 transition-colors"
        >
            ...
            <div key={index} className="flex items-center gap-4 group">
              <div className="flex-1">
                <input
                  type="text"
                  value={entry.cause}
                  onChange={(e) => updateEntry(index, "cause", e.target.value)}
                  placeholder="Cause"
                  className="w-full bg-transparent border-b border-gray-300 px-0 py-2 text-gray-700 focus:border-blue-500 outline-none placeholder-gray-300 transition-colors"
                />
              </div>
              ...
              <div className="flex-1">
                <input
                  type="text"
                  value={entry.effect}
                  onChange={(e) => updateEntry(index, "effect", e.target.value)}
                  placeholder="Effect"
                  className="w-full bg-transparent border-b border-gray-300 px-0 py-2 text-gray-700 focus:border-blue-500 outline-none placeholder-gray-300 transition-colors"
                />
              </div>
      ...
      <div className=" bottom-0 left-0 right-0 p-6 sm:px-12 sm:py-8 flex items-center justify-end max-w-7xl mx-auto w-full pointer-events-none">
        <div className="pointer-events-auto flex gap-4">
          ...
          {/* TODO: Navigate to next Question5 when available */}
          
          <button
            onClick={() => navigate("/dashboard/portfolio/question5")}
            ...
            >
              <div className="bg-white rounded-2xl w-full max-w-lg p-6 sm:p-8 shadow-2xl pointer-events-auto relative">
                ...
                <p className="text-gray-600 leading-relaxed mb-6 text-sm">
                  Add description for Cause and Effect here.
                </p>
